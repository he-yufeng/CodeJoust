...
MVP 只支持 Claude Code 和 aider，因为它们的 headless 模式和 usage 输出最稳。Phase 2 路线图（按优先级）：
...
- **v0.1**（当前）：Claude Code + aider，客观打分，HTML 报告。
- **v0.2**：Codex CLI + Gemini CLI 适配器；`--judge` 用 LLM-as-judge 给打平的情况加权；YAML 配置保存常用 agent profile。
