...
> **关于版本**。当前 PyPI 版本（`codejoust 0.1.0`）只含 Claude Code 和 aider。Codex 和 Gemini adapter 已在 `main`，下一次发 PyPI 一起带上。想立刻尝鲜：
>
> ```bash
...
- **v0.1.1**：OpenAI Codex CLI 适配器——三家同台开箱可用。
- **v0.2.0**（当前）：Google Gemini CLI 适配器；四家同时跑同一任务。
- **v0.3**：`--judge` LLM-as-judge 给打平加权；YAML 配置保存 agent profile；Cursor + OpenHands 适配器；批量模式；导出 Markdown 贴 PR 描述。
